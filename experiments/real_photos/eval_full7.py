...
for fn in ["kodim02.png","kodim05.png","kodim13.png","kodim19.png"]:
    rgb=np.array(Image.open(os.path.join(d,fn)).convert("RGB"))
    H,W,_=rgb.shape
    r=encode_image(rgb,tau=0)
    tb=0; modes=[]
    for ch in range(3):
        cb=channel_bytes_v2(r["res"][:,:,ch].reshape(-1)); tb+=cb["best"]; modes.append(cb["mode"])
    nb=(H//16+1)*(W//16+1); hdr=nb*2+r["nlpc"]*3*8+48*8+48
    tot=(tb+hdr+7)//8; bpp=tot*8/(H*W*3)
    print(f"{fn} V2bpp={bpp:.2f} V2bytes={tot} modes={modes} ok={r['lossless_ok']}",flush=True)