%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% HAPRE-CROWN DCC 2027 submission
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\author{%
Hassen Hamdi\\[0.25em]
{\small Independent Research} \\ {\small \url{hassenhamdi12@gmail.com}}
}
...
\begin{abstract}
We report a lossless image-compression campaign from a tiny
codec to CROWN6, an exact integer codec at 3.1978 bits per
subpixel on seven Kodak images, with byte-asserted round-trips. Seven
widely-used baselines fall to paired Wilcoxon tests, while FLIF and JPEG-XL
at two efforts stand — losses disclosed at the same rigor as wins. A stacked classical ceiling with leave-one-out marginals
marks where hand contexts end, and a mechanism-backed negative atlas maps
where not to drill. Code, streams, and data tables:
\url{https://github.com/hassenhamdi/hapre-crown}.
\end{abstract}
...
load-bearing digits independently recomputed before submission.
...
Huffman the bias adapter is empirically null (ablation study, $\sim$0 over 7 images and 0/126 cells). Measured JPEG-LS (CharLS 2.4.4, RGB) scores 4.5757 (E) on our seven;
...
2.72, JPEG-XL 2.63--2.88 (version spread; ours e9 3.03 sits above that
literature spread on our subset with cjxl 0.11.2). We do not claim the neural SOTA. We claim the
strongest fully-counted classical-plus-micro-neural exact codec in this
campaign: 60.7\% of the way from JXL-e1 (3.72) to DLPR (2.86).
...
regression (overfit); e8$\to$e9 $-0.4\%$ (4.4$\times$ time). Weaknesses: wider groups help everywhere and a single fixed RCT beats the
search; tuned-e9 reaches $\approx$2.94. The full ladder, knock-outs, and repro commands ship with the code.
...
RCT dead (fragmentation). \textbf{Borders:} encoder $==$ decoder exactly
on every path (audited).

\SubSection{Prediction: MED to E20}
LOCO-365 contexts; auto-K/WIDE-K quantile clustering vs GRID dictionary,
per-channel exact-byte competition. Expert bank:
...
\begin{table}[h]
\centering\small
\caption{Champion lineage (each step exact-gated; CROWN6 ties CROWN4: W=6, p=.81).}
\label{tab:lineage}
...
200 iters, adaptive-int16, double MDL gate).
Wire-forward bit-exact by construction: frozen tanh LUT (step $2^{-14}$,
\texttt{-ffp-contract=off}), exact int16 dequant, half-up rounding. C-vs-wire
xcheck: 0 mismatches / 17.2M px (E). Side: 64\,B thresholds + 9 scale-id
bytes + 226\,B/winning net; per-channel dual-track gate keeps CROWN6 within
sub-0.0015 exact bytes of CROWN4 per image (observed deltas $-0.0111$ to
$+0.0008$; tie W=6 p=.81, losses on 07/13/23 disclosed).
...
\texttt{decode==original} byte-compared. Full byte tables are specified in
the release (CROWN6 format, plus CROWN4 for the shared sections);
the C++ port reproduces reference streams sha-identically on all
7 Kodak images given the released weights with a 9-way parallel encoder ($\sim$27\,s/img, $\sim$117\,s
sequential) and a pure-C++ trainer.
...
Setup: Kodak 7 (768$\times$512$\times$3, $N$=1179648). Baselines on the
same machine: PIL 12.2.0 (PNG/WebP), cjxl/djxl 0.11.2 (4 threads), FLIF v0.4,
CharLS 2.4.4, QOI. Ours: gcc 15.2.1, Python 3.14.2 / numpy 2.4.6 /
torch 2.14 CPU (MLP training only); C++ encode 9-way parallel by default.
bpp = stream\_bytes$\cdot$8/$N$.
All claims Wilcoxon paired two-sided ($n=7$; pre-registered per-boss
comparisons without multiplicity correction; $n=7$ floor $p=.016$).
Round-trips byte-asserted (E) or scalar-sim + table proofs (P).
Determinism: fixed per-unit seeds; C++ port bit-deterministic given released
weights (frozen LUT, fixed accumulation order, strict join order).
Reported streams use single-seed weights.
...
FLIF v0.4 & 2.9699 & $+7.7\%$ & 0W-7L p=.016 & STANDING \\
...
configs. CROWN-huff ties PIL-m3 and CROWN6 ties-or-betters m6 (no 7/7
overclaim). Tuned-e9 (2.9452) is the true Boss-6 bar.

\begin{table}[h]
\centering\small
\caption{Per-image bpp (exact bytes, all codecs).}
\label{tab:perimage}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
% F1b heatmap dropped from the paper (duplicates Table 5 numbers).
% \begin{figure}[ht]
% \centering
...
Marginals do not sum, and the leftover unstacked gains are bounded $\approx-0.01$,
so classical stacking cannot reach e3. The remaining gap is representational
...
parallel, sha-identical at any job count, decode 182--348\,ms;
trainer $\sim$13\,s/img pure-C++ (no torch at runtime). Near-lossless
prototype: $\tau$1 $\sim$2.75--3.23 bpp at maxerr 3; $\tau$2 $\sim$2.0--2.6
at maxerr 4.

\Section{Negative Results}

Each row lists the config, the delta, and the mechanism. Suffix (P) marks
probe-level numbers; (E) marks exact-codec numbers.

\begin{figure}[ht]
\centering
\includegraphics[width=\figurewidth]{Figures/F4_negative_atlas.pdf}
\caption{Negative atlas (selected). Each bar vs its comparison baseline;
red = loss, green = win retired in-frame.}
\label{fig:negatives}
\end{figure}

\begin{table}[h]
\centering\small
\caption{Dead-end mechanisms (abridged; details ship with the code).}
\label{tab:negatives}
\begin{tabular}{@{}llp{2.86in}@{}}
\toprule
...
Bands / bias-Huffman & $\sim$0 / empirically 0 & fragmentation; translation-invariance \\
...
Class & Fix \\
\midrule
Causality peeks & causal zero-border taps \\
Empty-group framing & run semantics; empty payloads skipped \\
Alphabet clipping & $\pm$1024 bounds; loud asserts, decoder never clips \\
rANS precision & $M=14$ ($2^M\gg A$) \\
Border handling & encoder $==$ decoder on every path \\
Unary polarity splits & pinned packer per generation \\
tanh 1-ulp & frozen tanh LUT; 0/17.2M cross-check \\
Quantizer scale & detect all-zero weights; rerun \\
...
\textbf{Texture gap.} CROWN6 leads on average yet loses kodim05 (+1.95\% vs
JXL-e3) and kodim13 (+1.00\% vs JXL-e3) (E); FLIF takes all rows. The remaining
path is larger-neighborhood prediction, not finer coding. \textbf{Held-out.}
Three unseen Kodak images (04/15/24): RUN passes at 3.39/3.30/3.60 and CROWN6
passes on kodim15 at 2.83, matching the Python reference. \textbf{Learned scale.}
The true e9 bar ($\approx$2.94) needs learned contexts at JXL scale.
\textbf{Speed and robustness.} Encoding is slow (ratio game only); noisy-input
...
Per IEEE policy we disclose AI assistance: an agentic coding assistant
(OpenCode harness, Muse Spark) carried out the work under human guidance —
implementing the codec in C and C++, running the full
experimentation cycles, executing all verification (byte-asserted round-trips,
recomputed statistics, full test battery), generating the figures,
and drafting all paper sections. The human author corrected and guided
the AI's direction throughout and takes full responsibility for correctness
and originality.
