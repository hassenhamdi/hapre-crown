...
(\texttt{CHECKSUMS.txt}, \texttt{docs/VERIFY\_report.md}, \texttt{src/*\_FORMAT.md};
...
the code in \texttt{src/CROWN6\_FORMAT.md} (plus \texttt{src/CROWN4\_FORMAT.md} for the
shared sections);
...
(\texttt{cjxl -d 0 -e 9 -g 3 -I 100 -E 11}, 7/7 round-trip PASS,
\texttt{docs/EVIDENCE\_2026-09-10.md}) is the true Boss-6 bar.
...
Baselines with absolute bpp are banked per-row in \texttt{docs/ATLAS\_BASELINES.md}.
...
\begin{table}[h]
\centering\small
\caption{Dead-end mechanisms (abridged; per-row baselines in \texttt{docs/ATLAS\_BASELINES.md}, full text in probe ledgers).}
\label{tab:negatives}
...
416939\,B byte-identical to the Python reference
(\texttt{docs/EVIDENCE\_2026-09-10.md}). \textbf{Learned scale.} True e9 bar $\approx$2.94 needs
...
Per IEEE policy we disclose AI assistance: agentic coding assistants
(OpenCode harness, Claude model family) carried out the work under human
supervision — implementing the \texttt{src/} and \texttt{cpp/} codec code,
designing and running the full experimentation cycles (probes, training,
measurements), writing the figure-generation script, and drafting all paper
sections. The human author set the goals and methods, ran all verification
(byte-asserted round-trips, recomputed statistics, full test battery), and
takes full responsibility for correctness and originality.
