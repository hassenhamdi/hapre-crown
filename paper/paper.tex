...
where not to drill. All streams, code, and ledgers are banked in the
reproducibility supplement; the code repository will be made public
at camera-ready.
\end{abstract}
...
the code (CROWN6\_FORMAT.md, plus CROWN4\_FORMAT.md for the shared sections;
both in the supplement);
C++ port (\texttt{cpp/}) reproduces reference streams sha-identically on all
7 Kodak images given the banked weights with a 9-way parallel encoder ($\sim$27\,s/img, $\sim$117\,s
...
(\texttt{CROWN\_JOBS=1} forces sequential for debugging). bpp = stream\_bytes$\cdot$8/$N$.
...
(\texttt{cjxl -d 0 -e 9 -g 3 -I 100 -E 11}, 7/7 round-trip PASS) is the true Boss-6 bar.
...
Baselines with absolute bpp are banked per-row in the reproducibility
supplement (\texttt{ATLAS\_BASELINES.md}).
...
\begin{table}[h]
\centering\small
\caption{Dead-end mechanisms (abridged; per-row baselines in the supplement, full text in probe ledgers).}
\label{tab:negatives}
...
tanh 1-ulp & frozen LUT $2^{-14}$, \texttt{-ffp-contract=off}, 0/17.2M xcheck \\
...
JXL-e3) and kodim13 (+1.00\% vs JXL-e3) (E); FLIF takes all rows. Larger-neighborhood
...
vs test-7 3.5114); CROWN6 on kodim15 exposed two encoder bugs (since fixed:
lstsq degenerate fallback, LMS-residual assembly) and now round-trips at
416939\,B byte-identical to the Python reference. \textbf{Learned scale.} True e9 bar $\approx$2.94 needs
...
implementing the \texttt{src/} and \texttt{cpp/} codec code, running the full
experimentation cycles, executing all verification (byte-asserted round-trips,
recomputed statistics, full test battery), writing the figure-generation
script, and drafting all paper sections. The human author corrected and guided
